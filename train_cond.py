...
from models import ConditionalDiscriminator, ConditionalGenerator, weights_init
...
def main():
    ...
    # Models
    discriminator = ConditionalDiscriminator(n_channels=n_channels, n_classes=n_classes).to(device)
    generator = ConditionalGenerator(z_dim=z_dim, n_channels=n_channels, n_classes=n_classes).to(device)
    ...
    # Data
    data_location = '/home/itdfh/dev/GTA/data/age_prediction_up/age_prediction/train'
    trainset = dset.ImageFolder(root=data_location, transform=transform)
    # trainset = datasets.EMNIST('dataset', 
    #                            transform=transform, 
    #                            download=True,
    #                            split='letters')
    ...
    bce_loss = nn.BCELoss()
    writer = SummaryWriter(f"runs/AGE-GAN")
    ...
    for epoch in epoch_iterator:
        ...
        for batch_idx, (real, labels) in enumerate(batch_iterator):
            real = real.to(device)
            ...
            if batch_idx == 0:
                with torch.no_grad():
                    # Reshape to (B, C, H, W)
                    fake = generator(fixed_noise, labels).reshape(-1, n_channels, 64, 64)
